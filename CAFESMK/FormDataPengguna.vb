...
Public Class FormDataPengguna
    ...
    Private Sub FormDataPengguna_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing
        FormHalamanUtama.Show()
    End Sub

    Private Sub Button9_Click(sender As Object, e As EventArgs) Handles Button9.Click
        FormTambahPengguna.Show()
        Me.Hide()
    End Sub
    Sub start()
        Call Connection()
        da = New SqlDataAdapter("SELECT id_pengguna, nama, jenis_kelamin, alamat, no_hp, jenis FROM pengguna", conn)
        ds = New DataSet
        dt = New DataTable
        ds.Clear()
        da.Fill(dt)
    ...
    Private Sub Button8_Click(sender As Object, e As EventArgs) Handles Button8.Click
        If MessageBox.Show("Anda yakin ingin menghapus " & TextBox1.Text & "?", "Hapus data", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = ShowDialog.Yes Then
            Call Connection()
            Dim Hapus As String = "DELETE pengguna WHERE id_pengguna ='" & TextBox4.Text & "'"
    ...
    Private Sub Button7_Click(sender As Object, e As EventArgs) Handles Button7.Click
        ...
        Else
            Call Connection()
            Dim ubah As String = "UPDATE pengguna SET
                                  nama = '" & TextBox1.Text & "',
                                  jenis_kelamin = '" & ComboBox1.Text & "',
                                  alamat = '" & TextBox2.Text & "',
                                  no_hp = '" & TextBox3.Text & "',
                                  jenis = '" & ComboBox2.Text & "'
                                  WHERE id_pengguna = '" & TextBox4.Text & "'"
            cmd = New SqlCommand(ubah, conn)
            cmd.ExecuteNonQuery()
            If MessageBox.Show("Data Berhasil diubah", "Ubah Data", MessageBoxButtons.OK, MessageBoxIcon.Information) = DialogResult.OK Then
            End If
            Call start()
            Call empty()
        End If
